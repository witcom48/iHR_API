...
{
    public class cls_ctTRTimeshiftself
    {
        string Message = string.Empty;

        cls_ctConnection Obj_conn = new cls_ctConnection();

        public cls_ctTRTimeshiftself() { }
        ...
        private List<cls_TRTimeshiftself> getData(string condition)
        {
            List<cls_TRTimeshiftself> list_model = new List<cls_TRTimeshiftself>();
            cls_TRTimeshiftself model;
            try
            {
        ...
        public List<cls_TRTimeshiftself> getDataByFillter(int id, int status, string com, string emp, DateTime datefrom, DateTime dateto)
        {
            string strCondition = "";
            if (!id.Equals(0))
        ...
        public cls_TRTimeshiftself getDataByID(int id)
        {

            string strCondition = " AND SELF_TR_TIMESHIFT.TIMESHIFT_ID='" + id + "'";

            List<cls_TRTimeshiftself> list_model = this.getData(strCondition);
        ...
        public string insert(cls_TRTimeshiftself model)
        {
            string blnResult = "";
            try
            {
        ...
        public string update(cls_TRTimeshiftself model)
        {
            string blnResult = "";
            try
            {
